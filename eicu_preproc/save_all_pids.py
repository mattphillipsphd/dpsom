...
def save_all_pids(configs):
    ...
    df_patient = pd.read_hdf(configs["patient_table_path"],
            configs["generic_dset_id"], mode='r')
    ...
    df_async = pd.read_hdf(configs["vital_aper_path"],
            configs["generic_dset_id"], mode='r')
    ...
    df_sync = pd.read_hdf(configs["vital_per_path"],
            configs["generic_dset_id"], mode='r')
...
if __name__ == "__main__":
    ...
    # Input paths
    parser.add_argument("--patient_table_path",
            default=pj(HOME, "Datasets/eicu-2.0/hdf/patient.h5"),
                help="Location of patient table")
    parser.add_argument("--vital_aper_path",
            default=pj(HOME, "Datasets/eicu-2.0/hdf/vitalAperiodic.h5"),
            help="Path of vital aperiodic table") 
    parser.add_argument("--vital_per_path",
            default=pj(HOME, "Datasets/eicu-2.0/hdf/vitalPeriodic.h5"),
            help="Path of vital periodic table")

    # Output paths
    parser.add_argument("--pid_stay_list",
            default=pj(HOME, "Datasets/eicu-2.0/all_pid_stays.txt"),
            help="PID stay list to write")

    # Parameters
    parser.add_argument("--generic_dset_id", default="data",
            help="HDF data-set ID")
    ...
    configs["ASYNC_VITALS"] = ["noninvasivesystolic", "noninvasivediastolic",
            "noninvasivemean", "paop", "cardiacoutput", "cardiacinput",
            "svr", "svri", "pvr", "pvri"]

    configs["SYNC_VITALS"] = ["temperature", "sao2", "heartrate", "respiration",
